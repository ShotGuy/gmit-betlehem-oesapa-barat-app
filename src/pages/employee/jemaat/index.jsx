...
export default function EmployeeMembersManagement() {
    ...
    // Delete mutation - ONLY IF EMPLOYEE HAS DELETE PERMISSION (Usually not, but keeping logic just in case or allowed)
    ...
    const rowActions = [
        {
            label: "Detail",
            icon: Eye,
            onClick: (item) => router.push(`/employee/jemaat/${item.id}`),
            variant: "outline",
            tooltip: "Lihat detail lengkap jemaat",
        },
        // Edit - Check permission canManageJemaat
        ...(authData?.pegawai?.canManageJemaat
            ? [
                {
                    label: "Edit",
                    icon: Edit,
                    onClick: (item) => router.push(`/employee/jemaat/edit/${item.id}`),
                    variant: "outline",
                    tooltip: "Edit informasi jemaat",
                },
            ]
            : []),
        // Delete - Usually Employees cannot delete, but lets keep it if logic exists
        // Keeping commented out or highly restricted
        /* ...(authData?.isAdmin
          ? [
              {
                label: "Hapus",
    ...
    const headerActions = [
        ...
        // Only add this item if employee has canManageJemaat permission
        ...(authData?.pegawai?.canManageJemaat
            ? [
                {
                    label: "Tambah Jemaat",
                    icon: Plus,
                    onClick: () => router.push("/employee/jemaat/create"),
                    variant: "default",
                },
            ]
            : []),
    ...
    return (
        <>
            <PageTitle title="Daftar Jemaat - Staff" />
            {/* Page Header */}
